...
class CodeWorkflow:
    ...
    async def perform_diff(self, query: str) -> str:
        try:
            ...
            print(f"{GREEN}🔍 Processing query...")
            ...
            for i, chunk in enumerate(code_chunks, 1):
                print(f"{GREEN}  {i}. {chunk['type']} '{chunk['name']}' in {chunk['filename']} (similarity: {chunk['similarity']:.3f})")
            ...
            print(f"{GREEN}📝 Generating diff preview...")
            ...
            print(f"{GREEN}{'='*50}")
            print(f"{GREEN}PROPOSED CHANGES")
            print(f"{GREEN}{'='*50}")
            print(f"{GREEN}File: {best_chunk['filename']}")
            print(f"{GREEN}Target: {best_chunk['type']} '{best_chunk['name']}' (lines {best_chunk['lineno']}-{best_chunk['end_lineno']})")
            print(f"{GREEN}{'='*50}")
            print(f"{GREEN}Diff:")
            print(diff)
            print(f"{GREEN}{'='*50}")
            ...
            try:
                ...
                if permission == 'y' or permission == 'yes':
                    ...
                    return f"{GREEN}Changes cancelled by user."
...
{GREEN}File: {chunk['filename']}
{GREEN}Modified: {chunk['type']} '{chunk['name']}'
