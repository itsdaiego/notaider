...
class CodeWorkflow:
    ...
    async def perform_diff(self, query: str) -> str:
        try:
            ...
            Please analyze which chunks need to be modified based on the user request.
            For EACH chunk that needs modification, return a CodeWorkflowOutput with:
            - code: the transformed code for that specific chunk
            - filename: the filename of that chunk

            IMPORTANT: Only return entries for chunks that actually need to be changed.
            If the user request only targets one specific function/class, only return one entry for that function/class.

            IMPORTANT: If the chunk given does NOT need any changes, you should NOT return an entry for it.
            """
